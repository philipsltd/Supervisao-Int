...
solve(Goals,W0, W3, Forbidden, Plan, Domain):-
    ...
    % se o Domain n�o estiver instanciada, ent�o s�o unificadas.
    % Em caso disso, Domain assume o valor que est� nessa accao
...
/*
      ...
      */
       ...
       SequenceTermOrList   =.. [seq | _],
       findall( Precc, arg(_, SequenceTermOrList, Precc), List),
       flatten(List, LList),
       subset( W1, LList) % troca-se pois numa seq podem haver estados contraditorios.
    ),
...
% a accao n�o tem domain, ent�o pode ser usada
