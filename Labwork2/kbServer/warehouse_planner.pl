...
init:-
    assert_state(stored(0)),
    assert_state(inStock(0)),
    assert_state(withdrawn(0)),
    state(x_moving(X)),
    state(y_moving(Y)),
    state(z_moving(Z)),
...
start_retrieve_place_cell_sequential_plan(X, Z):-
    ...
    -> 
        S2 is S+1,
        retract_state(stored(_)),
        assert_state(stored(S2))
    ;
        assert_state(stored(1))
    ),
    (
        state(inStock(IS))
    -> 
        IS2 is IS+1,
        retract_state(inStock(_)),
        assert_state(inStock(IS2))
    ;
        assert_state(inStock(1))
    ).
...
start_retrieve_place_right_sequential_plan(X, Z):-
    ...
    -> 
        W2 is W+1,
        retract_state(withdrawn(_)),
        assert_state(withdrawn(W2))
    ;
        assert_state(withdrawn(1))
    ).

retrive_from_1_1:-
    start_retrieve_place_cell_sequential_plan(1,1.0).
